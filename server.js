...
const COLA_CONSULTAS = Symbol('colaConsultas');
...
function ejecutarConsulta(db, consulta, parametros = []) {
  if (!db[COLA_CONSULTAS]) {
    db[COLA_CONSULTAS] = Promise.resolve();
  }

  const ejecutar = () => new Promise((resolve, reject) => {
  ...
  const colaActual = db[COLA_CONSULTAS].catch(() => undefined);
  const resultado = colaActual.then(() => ejecutar());
  db[COLA_CONSULTAS] = resultado.catch(() => undefined);
  return resultado;
}
...
async function obtenerDefinicionCamposTabla(db, nombreTabla) {
  ...
  const consulta = `
    SELECT TRIM(R.RDB$FIELD_NAME) AS CAMPO,
           ...
           F.RDB$FIELD_PRECISION AS PRECISION_CAMPO,
  ...
  registros.forEach((registro) => {
    ...
    campos[clave] = {
      ...
      precision: Number(registro.PRECISION_CAMPO) || null,
      escala
    };
  });
